task.wait(0.1)
-- ============================================================
-- BIXIE Hub Best Duel Script
-- Execute in Private Server First Wait 4-5 Minute To Duel
-- ============================================================
...
local ConfigFileName = "kizzi_hub_Config.json"
...
pcall(function()
    if readfile and isfile and isfile(ConfigFileName) then
        ...
        if data then
            ...
            if data.KEY_DROP then KEYBINDS.DROP = Enum.KeyCode[data.KEY_DROP] end
...
local function SaveConfig()
    ...
    data.KEY_DROP = KEYBINDS.DROP.Name
...
-- ============================================================
-- DROP (R) - Saute vers le haut puis redescend vite
-- ============================================================
local function doDrop()
    pcall(function()
        ...
        -- Attendre un court instant puis redescendre vite
...
coordESPFolder.Name = "kizzi_CoordESP"
...
local function startAutoWalk()
    ...
    autoWalkPhase = 1
    local savedSpeed = Values.BoostSpeed
    Values.BoostSpeed = 60  -- force speed à 60
    autoWalkConnection = RunService.Heartbeat:Connect(function()
        if not AutoWalkEnabled then
            Values.BoostSpeed = savedSpeed  -- restore speed après
            return
        end
        local c = Player.Character
        if not c then return end
...
-- ============================================================
-- GUI - KIZZI HUB (taille réduite de 40%)
-- ============================================================
local isMobile = UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled
-- guiScale reste à 1 pour que les pixels soient corrects
local guiScale = isMobile and 0.4 or 1

local C = {
    bg = Color3.fromRGB(10, 10, 14),
    bgRow = Color3.fromRGB(14, 14, 20),
    purple = Color3.fromRGB(138, 43, 226),
    purpleLight = Color3.fromRGB(170, 100, 255),
    purpleDark = Color3.fromRGB(90, 20, 160),
    accent = Color3.fromRGB(138, 43, 226),
    text = Color3.fromRGB(230, 230, 240),
    textDim = Color3.fromRGB(140, 120, 180),
    success = Color3.fromRGB(34, 197, 94),
    danger = Color3.fromRGB(239, 68, 68),
    border = Color3.fromRGB(100, 40, 180),
    sliderFill = Color3.fromRGB(138, 43, 226),
    sliderBg = Color3.fromRGB(30, 20, 50),
    keyBg = Color3.fromRGB(40, 20, 80),
...
progressBar.Position = UDim2.new(0.5, -200 * guiScale, 1, -80 * guiScale)
...
ProgressLabel.Size = UDim2.new(0.4, 0, 0.55, 0)
ProgressLabel.Position = UDim2.new(0, 10 * guiScale, 0, 4 * guiScale)
...
ProgressPercentLabel.Size = UDim2.new(1, 0, 0.55, 0)
...
RadiusInput.Position = UDim2.new(1, -46 * guiScale, 0, 4 * guiScale)
RadiusInput.BackgroundColor3 = C.sliderBg
...
pTrack.Size = UDim2.new(0.94, 0, 0, 7 * guiScale)
pTrack.Position = UDim2.new(0.03, 0, 1, -13 * guiScale)
...
-- ============================
-- MAIN WINDOW (réduit de 40%)
-- ============================
local MAIN_W = 380  -- taille correcte
local MAIN_H = 520  -- hauteur correcte

local main = Instance.new("Frame", sg)
main.Name = "Main"
main.Size = UDim2.new(0, MAIN_W, 0, MAIN_H)
main.Position = isMobile and UDim2.new(0.5, -MAIN_W/2, 0.5, -MAIN_H/2) or UDim2.new(1, -MAIN_W - 20, 0, 20)
main.BackgroundColor3 = C.bg
main.BorderSizePixel = 0
main.Active = true
main.Draggable = true
main.ClipsDescendants = true
Instance.new("UICorner", main).CornerRadius = UDim.new(0, 14 * guiScale)
...
header.Size = UDim2.new(1, 0, 0, 54 * guiScale)
header.BackgroundTransparency = 1
header.ZIndex = 3

local titleLabel = Instance.new("TextLabel", header)
titleLabel.Size = UDim2.new(1, -80 * guiScale, 1, 0)
titleLabel.Position = UDim2.new(0, 14 * guiScale, 0, 0)
titleLabel.BackgroundTransparency = 1
titleLabel.Text = "KIZZI HUB"
...
settingsBtn.Size = UDim2.new(0, 30 * guiScale, 0, 30 * guiScale)
settingsBtn.Position = UDim2.new(1, -72 * guiScale, 0.5, -15 * guiScale)
settingsBtn.BackgroundColor3 = C.keyBg
settingsBtn.Text = "⚙"
settingsBtn.TextColor3 = C.purpleLight
settingsBtn.Font = Enum.Font.GothamBold
settingsBtn.TextSize = 16 * guiScale
settingsBtn.ZIndex = 5
Instance.new("UICorner", settingsBtn).CornerRadius = UDim.new(0, 8 * guiScale)

-- Bouton flottant pour rouvrir le GUI (déclaré avant closeBtn)
local reopenBtn = Instance.new("TextButton", sg)
reopenBtn.Size = UDim2.new(0, 38, 0, 38)
reopenBtn.Position = UDim2.new(1, -58, 0, 20)
reopenBtn.BackgroundColor3 = C.purple
...
closeBtn.Position = UDim2.new(1, -36 * guiScale, 0.5, -15 * guiScale)
...
sep.Size = UDim2.new(1, -20 * guiScale, 0, 1)
sep.Position = UDim2.new(0, 10 * guiScale, 0, 54 * guiScale)
sep.BackgroundColor3 = C.border
sep.BorderSizePixel = 0
sep.ZIndex = 3

-- Two columns
local leftSide = Instance.new("ScrollingFrame", main)
leftSide.Size = UDim2.new(0.5, -6, 1, -62 * guiScale)
leftSide.Position = UDim2.new(0, 4, 0, 60 * guiScale)
leftSide.BackgroundTransparency = 1
leftSide.BorderSizePixel = 0
leftSide.ScrollBarThickness = 3
leftSide.ScrollBarImageColor3 = C.purple
leftSide.CanvasSize = UDim2.new(0, 0, 0, 0)
leftSide.AutomaticCanvasSize = Enum.AutomaticSize.Y
leftSide.ZIndex = 2
...
local rightSide = Instance.new("ScrollingFrame", main)
rightSide.Size = UDim2.new(0.5, -6, 1, -62 * guiScale)
rightSide.Position = UDim2.new(0.5, 2, 0, 60 * guiScale)
rightSide.BackgroundTransparency = 1
rightSide.BorderSizePixel = 0
rightSide.ScrollBarThickness = 3
rightSide.ScrollBarImageColor3 = C.purple
rightSide.CanvasSize = UDim2.new(0, 0, 0, 0)
rightSide.AutomaticCanvasSize = Enum.AutomaticSize.Y
rightSide.ZIndex = 2
...
local ROW_H = 36 * guiScale    -- légèrement réduit (était 42)
local SLIDER_H = 46 * guiScale -- légèrement réduit (était 52)
...
local function createToggleWithKey(parent, labelText, keybindKey, enabledKey, callback, specialColor, order)
    ...
    Instance.new("UICorner", row).CornerRadius = UDim.new(0, 7 * guiScale)

    local keyBtn = Instance.new("TextButton", row)
    keyBtn.Size = UDim2.new(0, 28 * guiScale, 0, 22 * guiScale)
    keyBtn.Position = UDim2.new(0, 5 * guiScale, 0.5, -11 * guiScale)
    keyBtn.BackgroundColor3 = C.keyBg
    keyBtn.Text = KEYBINDS[keybindKey].Name
    keyBtn.TextColor3 = C.purpleLight
    keyBtn.Font = Enum.Font.GothamBold
    keyBtn.TextSize = 10 * guiScale
    keyBtn.ZIndex = 4
    ...
    label.Position = UDim2.new(0, 38 * guiScale, 0, 0)
    ...
    toggleBg.Size = UDim2.new(0, 44 * guiScale, 0, 24 * guiScale)
    toggleBg.Position = UDim2.new(1, -50 * guiScale, 0.5, -12 * guiScale)
    toggleBg.BackgroundColor3 = defaultOn and onColor or C.toggleOff
    toggleBg.ZIndex = 4
    Instance.new("UICorner", toggleBg).CornerRadius = UDim.new(1, 0)

    local toggleCircle = Instance.new("Frame", toggleBg)
    toggleCircle.Size = UDim2.new(0, 18 * guiScale, 0, 18 * guiScale)
    toggleCircle.Position = defaultOn and UDim2.new(1, -21 * guiScale, 0.5, -9 * guiScale) or UDim2.new(0, 3 * guiScale, 0.5, -9 * guiScale)
    toggleCircle.BackgroundColor3 = Color3.new(1, 1, 1)
    toggleCircle.ZIndex = 5
    Instance.new("UICorner", toggleCircle).CornerRadius = UDim.new(1, 0)

    local clickBtn = Instance.new("TextButton", row)
    clickBtn.Size = UDim2.new(0.55, 0, 1, 0)
    ...
    local function setVisual(state, skipCallback)
        isOn = state
        TweenService:Create(toggleBg, TweenInfo.new(0.2), {BackgroundColor3 = isOn and onColor or C.toggleOff}):Play()
    ...
    keyBtn.MouseButton1Click:Connect(function()
        ...
        keyBtn.Text = "..."
        playSound("rbxassetid://6895079813", 0.25, 1.5)
    end)
...
local function createSlider(parent, labelText, minVal, maxVal, valueKey, callback, order)
    ...
    container.Size = UDim2.new(1, 0, 0, SLIDER_H)
    container.BackgroundColor3 = C.bgRow
    container.BorderSizePixel = 0
    container.ZIndex = 3
    container.LayoutOrder = order or 0
    Instance.new("UICorner", container).CornerRadius = UDim.new(0, 7 * guiScale)
    ...
    valueInput.Size = UDim2.new(0, 42 * guiScale, 0, 20 * guiScale)
    valueInput.Position = UDim2.new(1, -48 * guiScale, 0, 2 * guiScale)
    valueInput.BackgroundColor3 = C.sliderBg
    valueInput.Text = tostring(defaultVal)
    valueInput.TextColor3 = C.purpleLight
    valueInput.Font = Enum.Font.GothamBold
    valueInput.TextSize = 11 * guiScale
    valueInput.ClearTextOnFocus = false
    valueInput.ZIndex = 4
    Instance.new("UICorner", valueInput).CornerRadius = UDim.new(0, 5 * guiScale)

    local sliderBg = Instance.new("Frame", container)
    sliderBg.Size = UDim2.new(0.9, 0, 0, 8 * guiScale)
    sliderBg.Position = UDim2.new(0.05, 0, 0, 30 * guiScale)
    ...
    thumb.Position = UDim2.new(math.clamp(pct,0,1), -7 * guiScale, 0.5, -7 * guiScale)
    ...
    valueInput.FocusLost:Connect(function()
        ...
        if n then
            ...
            thumb.Position = UDim2.new(r, -7 * guiScale, 0.5, -7 * guiScale)
            Values[valueKey] = n
            callback(n)
        end
    end)
...
-- ============================
createToggleWithKey(leftSide, "Speed Boost", "SPEED", "SpeedBoost", function(s)
    Enabled.SpeedBoost = s
    if s then startSpeedBoost() else stopSpeedBoost() end
end, nil, 1)

createSlider(leftSide, "Boost Speed", 1, 70, "BoostSpeed", function(v) Values.BoostSpeed = v end, 2)

createToggle(leftSide, "Anti Ragdoll", "AntiRagdoll", function(s)
    Enabled.AntiRagdoll = s
    if s then startAntiRagdoll() else stopAntiRagdoll() end
end, nil, 3)

createToggle(leftSide, "Auto Steal", "AutoSteal", function(s)
    Enabled.AutoSteal = s
    if s then startAutoSteal() else stopAutoSteal() end
end, nil, 4)

createToggleWithKey(leftSide, "Bat Aimbot", "BATAIMBOT", "BatAimbot", function(s)
    Enabled.BatAimbot = s
    if s then startBatAimbot() else stopBatAimbot() end
end, Color3.fromRGB(239, 68, 68), 5)

createToggleWithKey(leftSide, "Galaxy Mode", "GALAXY", "Galaxy", function(s)
    Enabled.Galaxy = s
    if s then startGalaxy() else stopGalaxy() end
end, nil, 6)

createSlider(leftSide, "Gravity %", 25, 130, "GalaxyGravityPercent", function(v)
    Values.GalaxyGravityPercent = v
    if galaxyEnabled then adjustGalaxyJump() end
end, 7)

createSlider(leftSide, "Hop Power", 10, 80, "HOP_POWER", function(v) Values.HOP_POWER = v end, 8)

-- ============================
-- POPULATE RIGHT COLUMN
-- ============================
createToggle(rightSide, "Speed While Stealing", "SpeedWhileStealing", function(s)
    Enabled.SpeedWhileStealing = s
    if s then startSpeedWhileStealing() else stopSpeedWhileStealing() end
end, nil, 1)

createSlider(rightSide, "Steal Speed", 10, 35, "StealingSpeedValue", function(v) Values.StealingSpeedValue = v end, 2)

createToggle(rightSide, "Optimizer + XRay", "Optimizer", function(s)
    Enabled.Optimizer = s
    if s then enableOptimizer() else disableOptimizer() end
end, nil, 3)

createToggle(rightSide, "Galaxy Sky Bright", "GalaxySkyBright", function(s)
    Enabled.GalaxySkyBright = s
    if s then enableGalaxySkyBright() else disableGalaxySkyBright() end
end, Color3.fromRGB(180, 80, 255), 4)

createToggle(rightSide, "Unwalk", "Unwalk", function(s)
    Enabled.Unwalk = s
    if s then startUnwalk() else stopUnwalk() end
end, nil, 5)

createToggleWithKey(rightSide, "Il ne reste plus", "AUTOLEFT", "AutoWalkEnabled", function(s)
    AutoWalkEnabled = s
    Enabled.AutoWalkEnabled = s
    if s then startAutoWalk() else stopAutoWalk() end
end, Color3.fromRGB(100, 150, 255), 6)

createToggleWithKey(rightSide, "Auto Right", "AUTORIGHT", "AutoRightEnabled", function(s)
    AutoRightEnabled = s
    Enabled.AutoRightEnabled = s
    if s then startAutoRight() else stopAutoRight() end
end, Color3.fromRGB(100, 220, 180), 7)

-- Save Button
local SaveRow = Instance.new("Frame", rightSide)
SaveRow.Size = UDim2.new(1, 0, 0, 44 * guiScale)
SaveRow.BackgroundTransparency = 1
SaveRow.LayoutOrder = 10

local SaveBtn = Instance.new("TextButton", SaveRow)
SaveBtn.Size = UDim2.new(1, -4, 0, 38 * guiScale)
SaveBtn.Position = UDim2.new(0, 2, 0.5, -19 * guiScale)
...
-- ============================
-- SETTINGS PANEL (réduit de 40%)
-- ============================
local SET_W = 260  -- settings panel taille correcte
local SET_H = 400  -- hauteur correcte

local settingsPanel = Instance.new("Frame", sg)
settingsPanel.Name = "SettingsPanel"
settingsPanel.Size = UDim2.new(0, SET_W, 0, SET_H)
settingsPanel.Position = UDim2.new(1, -SET_W - 20, 0, 20)
...
Instance.new("UICorner", settingsPanel).CornerRadius = UDim.new(0, 14 * guiScale)
local setStroke = Instance.new("UIStroke", settingsPanel)
setStroke.Thickness = 2
setStroke.Color = C.border

-- Settings header
local setHeader = Instance.new("Frame", settingsPanel)
setHeader.Size = UDim2.new(1, 0, 0, 50 * guiScale)
setHeader.BackgroundTransparency = 1
setHeader.ZIndex = 11

local setTitle = Instance.new("TextLabel", setHeader)
setTitle.Size = UDim2.new(1, -40 * guiScale, 1, 0)
setTitle.Position = UDim2.new(0, 14 * guiScale, 0, 0)
setTitle.BackgroundTransparency = 1
setTitle.Text = "KIZZI HUB"
setTitle.TextColor3 = C.text
setTitle.Font = Enum.Font.GothamBlack
setTitle.TextSize = 20 * guiScale
setTitle.TextXAlignment = Enum.TextXAlignment.Left
setTitle.ZIndex = 12

local setClose = Instance.new("TextButton", setHeader)
setClose.Size = UDim2.new(0, 28 * guiScale, 0, 28 * guiScale)
setClose.Position = UDim2.new(1, -34 * guiScale, 0.5, -14 * guiScale)
setClose.BackgroundTransparency = 1
setClose.Text = "×"
setClose.TextColor3 = C.textDim
setClose.Font = Enum.Font.GothamBold
setClose.TextSize = 20 * guiScale
setClose.ZIndex = 12
setClose.MouseButton1Click:Connect(function() settingsPanel.Visible = false end)
setClose.MouseEnter:Connect(function() setClose.TextColor3 = C.danger end)
setClose.MouseLeave:Connect(function() setClose.TextColor3 = C.textDim end)

local setSep = Instance.new("Frame", settingsPanel)
setSep.Size = UDim2.new(1, -20 * guiScale, 0, 1)
setSep.Position = UDim2.new(0, 10 * guiScale, 0, 50 * guiScale)
setSep.BackgroundColor3 = C.border
setSep.BorderSizePixel = 0
setSep.ZIndex = 11

-- Settings scroll
local setScroll = Instance.new("ScrollingFrame", settingsPanel)
setScroll.Size = UDim2.new(1, -8, 1, -58 * guiScale)
setScroll.Position = UDim2.new(0, 4, 0, 56 * guiScale)
setScroll.BackgroundTransparency = 1
setScroll.BorderSizePixel = 0
setScroll.ScrollBarThickness = 3
setScroll.ScrollBarImageColor3 = C.purple
...
setLayout.Padding = UDim.new(0, 3)
setLayout.SortOrder = Enum.SortOrder.LayoutOrder

-- Settings toggle helper
local function createSetToggle(parent, labelText, enabledKey, callback, order)
    local row = Instance.new("Frame", parent)
    row.Size = UDim2.new(1, 0, 0, ROW_H)
    row.BackgroundColor3 = C.bgRow
    row.BorderSizePixel = 0
    ...
    toggleBg.Size = UDim2.new(0, 44 * guiScale, 0, 24 * guiScale)
    toggleBg.Position = UDim2.new(1, -50 * guiScale, 0.5, -12 * guiScale)
    toggleBg.BackgroundColor3 = defaultOn and C.purple or C.toggleOff
    toggleBg.ZIndex = 13
    Instance.new("UICorner", toggleBg).CornerRadius = UDim.new(1, 0)
    ...
    local function setVisual(state, skipCallback)
        isOn = state
        TweenService:Create(toggleBg, TweenInfo.new(0.2), {BackgroundColor3 = isOn and C.purple or C.toggleOff}):Play()
        TweenService:Create(toggleCircle, TweenInfo.new(0.2, Enum.EasingStyle.Back), {
            Position = isOn and UDim2.new(1, -21 * guiScale, 0.5, -9 * guiScale) or UDim2.new(0, 3 * guiScale, 0.5, -9 * guiScale)
        }):Play()
        if not skipCallback then callback(isOn) end
...
local function createSetSlider(parent, labelText, minVal, maxVal, valueKey, callback, order)
    local container = Instance.new("Frame", parent)
    container.Size = UDim2.new(1, 0, 0, SLIDER_H)
    container.BackgroundColor3 = C.bgRow
    container.BorderSizePixel = 0
...
-- Settings keybind row helper
local function createSetKeyRow(parent, labelText, keybindKey, actionCallback, order)
    local row = Instance.new("Frame", parent)
    row.Size = UDim2.new(1, 0, 0, ROW_H)
    row.BackgroundColor3 = C.bgRow
    row.BorderSizePixel = 0
    ...
    if actionCallback then
        ...
        actBtn.Size = UDim2.new(0, 50 * guiScale, 0, 22 * guiScale)
        actBtn.Position = UDim2.new(1, -56 * guiScale, 0.5, -11 * guiScale)
        actBtn.BackgroundColor3 = C.purple
        actBtn.Text = "USE"
        actBtn.TextColor3 = Color3.new(1,1,1)
        actBtn.Font = Enum.Font.GothamBold
        actBtn.TextSize = 11 * guiScale
        actBtn.ZIndex = 13
        Instance.new("UICorner", actBtn).CornerRadius = UDim.new(0, 6 * guiScale)
        actBtn.MouseButton1Click:Connect(actionCallback)
    end
...
createSetSlider(setScroll, "Gravity %", 25, 130, "GalaxyGravityPercent", function(v)
    Values.GalaxyGravityPercent = v
    if galaxyEnabled then adjustGalaxyJump() end
end, 1)

createSetSlider(setScroll, "Hop Power", 10, 80, "HOP_POWER", function(v) Values.HOP_POWER = v end, 2)

createSetSlider(setScroll, "FOV", 50, 120, "FOV", function(v)
    Values.FOV = v
    setFOV(v)
end, 3)

createSetKeyRow(setScroll, "DROP (R)", "DROP", doDrop, 4)
createSetKeyRow(setScroll, "TP DOWN (A)", "TPDOWN", doTPDown, 5)

createSetSlider(setScroll, "Fall Speed", -500, 0, "FallSpeed", function(v) Values.FallSpeed = v end, 6)

createSetToggle(setScroll, "No Effect", "NoEffect", function(s)
    Enabled.NoEffect = s
    pcall(function()
        local c = Player.Character
        if not c then return end
...
createSetToggle(setScroll, "TP Ragdoll", "TPRagdoll", function(s)
    Enabled.TPRagdoll = s
    if s then enableTPRagdoll() end
end, 8)
...
task.spawn(function()
    ...
    if Values.FOV then setFOV(Values.FOV) end
end)
...
UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    if input.KeyCode == KEYBINDS.DROP then
        doDrop()
    end
